...
import { createLazyAcpRuntimeProxy, type CompleteAcpxRuntime } from './src/runtime-proxy.js';
...
type DeferredServiceState = {
  ...
  ownedRuntime: CompleteAcpxRuntime | null;
  params: CreateAcpxRuntimeServiceParams;
  realRuntime: CompleteAcpxRuntime | null;
  realService: OpenClawPluginService | null;
  startPromise: Promise<CompleteAcpxRuntime> | null;
  stopPromise: Promise<void> | null;
};

const loadServiceModule = createLazyRuntimeModule(() => import('./src/service.js'));

function unregisterOwnedRuntime(runtime: CompleteAcpxRuntime | null): void {
...
async function startRealService(
  ...
  lifecycleRevision: number,
  deferredRuntime: CompleteAcpxRuntime,
): Promise<CompleteAcpxRuntime> {
  ...
  state.startPromise = (async () => {
    let publishedRuntime: CompleteAcpxRuntime | null = null;
...
function createDeferredRuntime(
  ...
  const deferredRuntime: CompleteAcpxRuntime = createLazyAcpRuntimeProxy(
    (): Promise<CompleteAcpxRuntime> => startRealService(state, lifecycleRevision, deferredRuntime),
  );
