...
export const AcpxPluginConfigSchema = z.strictObject({
  ...
  startupProbe: z.boolean({ error: 'startupProbe must be a boolean' }).default(false),
  claudeExecutable: nonEmptyTrimmedString('claudeExecutable must be a non-empty string').optional(),
  permissionMode: z
  ...
  piSessionCatalog: z
    ...
    .optional(),
  diagnosticAgents: z
    .array(nonEmptyTrimmedString('diagnosticAgents entries must be non-empty strings'))
    .max(64, { error: 'diagnosticAgents must contain at most 64 entries' })
    .optional(),
