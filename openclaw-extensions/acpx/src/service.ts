...
import { createLazyAcpRuntimeProxy, type CompleteAcpxRuntime } from './runtime-proxy.js';
import {
...
type AcpxRuntimeLike = CompleteAcpxRuntime & {
...
type AcpxBackendLifecycle = {
  publish: (backend: { runtime: CompleteAcpxRuntime; healthy?: () => boolean }) => void;
  retract: (runtime: CompleteAcpxRuntime) => void;
};
...
function shouldProbeRuntimeAtStartup(
  configured: boolean,
  env: NodeJS.ProcessEnv = process.env,
): boolean {
  return configured && shouldRunStartupProbe(env) && env[SKIP_RUNTIME_PROBE_ENV] !== '1';
}
...
export function createAcpxRuntimeService(
  ...
  return {
    ...
    async start(ctx: OpenClawPluginServiceContext): Promise<void> {
      ...
      const shouldProbeRuntime = shouldProbeRuntimeAtStartup(pluginConfig.startupProbe);
