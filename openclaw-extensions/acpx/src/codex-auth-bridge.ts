...
import { copySourceCodexAuth } from './codex-auth-file.js';
import {
  extractTrustedCodexProjectPaths,
  renderIsolatedCodexConfig,
} from './codex-trust-config.js';
import { quoteCommandPart } from './command-line.js';
...
if (!command) {
  console.error(
    "[openclaw] bundled ${params.displayName} ACP adapter is missing; configure a local adapter command or rebuild the runtime",
  );
  process.exit(1);
}
...
function buildClaudeAcpWrapperScript(installedBinPath?: string, claudeExecutable?: string): string {
  return buildAdapterWrapperScript({
    displayName: 'Claude',
    installedBinPath,
    envSetup: `const env = {
  ...process.env,
  ${claudeExecutable ? `CLAUDE_CODE_EXECUTABLE: ${quoteCommandPart(claudeExecutable)},` : ''}
};`,
  });
}
...
async function writeClaudeAcpWrapper(
  baseDir: string,
  installedBinPath?: string,
  ...
  await fs.writeFile(wrapperPath, buildClaudeAcpWrapperScript(installedBinPath, claudeExecutable), {
    encoding: 'utf8',
  });
  await makeGeneratedWrapperExecutableIfPossible(wrapperPath);
  return wrapperPath;
...
function extractConfiguredAdapterArgs(params: {
  ...
  const configuredCommand = params.configuredCommand;
  if (!configuredCommand?.length) {
    return [];
  }
  const parts = configuredCommand;
  if (!parts.length) {
    return [];
  }
...
function buildCodexAcpWrapperCommand(wrapperPath: string, configuredCommand?: string[]): string[] {
  return configuredCommand?.length
    ? buildWrapperCommand(wrapperPath, [RUN_CONFIGURED_COMMAND_SENTINEL, ...configuredCommand])
    : buildWrapperCommand(wrapperPath);
}
...
export async function prepareAcpxCodexAuthConfig(params: {
  ...
  void params.logger;
  const wrapperBaseDir = path.join(params.stateDir, 'acpx');
  const enabledAgents = params.enabledAgents;
  const codexEnabled = !enabledAgents || enabledAgents.has('codex');
  const claudeEnabled = !enabledAgents || enabledAgents.has('claude');
  ...
  if (codexEnabled) {
    ...
    const wrapperPath = await writeCodexAcpWrapper(wrapperBaseDir, installedCodexBinPath);
    preparedAgents.codex = buildCodexAcpWrapperCommand(wrapperPath, configuredCodexCommand);
  }
  if (claudeEnabled) {
    const installedClaudeBinPath = await (
      params.resolveInstalledClaudeAcpBinPath ?? resolveInstalledClaudeAcpBinPath
    )();
    const claudeWrapperPath = await writeClaudeAcpWrapper(
      wrapperBaseDir,
      installedClaudeBinPath,
      params.pluginConfig.claudeExecutable,
    );
    preparedAgents.claude = buildClaudeAcpWrapperCommand(
      claudeWrapperPath,
