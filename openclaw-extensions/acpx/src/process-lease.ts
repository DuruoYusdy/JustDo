...
export function withAcpxLeaseEnvironment<T extends string | string[]>(params: {
  command: T;
  leaseId: string;
  gatewayInstanceId: string;
}): T {
  ...
  return appendAcpxLeaseArgs(
    params as { command: string; leaseId: string; gatewayInstanceId: string },
  ) as T;
}
