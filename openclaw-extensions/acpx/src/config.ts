...
export function resolveAcpxPluginConfig(params: {
  ...
  const pluginRoot = resolveAcpxPluginRoot(params.moduleUrl);
  const agentCommandPaths = {
    nodeExecutable: process.execPath,
    pluginRoot,
    openClawRoot: resolveOpenClawRoot(pluginRoot),
  };
  const agents = Object.fromEntries(
    Object.entries(normalized.agents ?? {}).flatMap(([name, entry]) => {
      const normalizedName = normalizeLowercaseStringOrEmpty(name);
      return normalizedName
        ? [
            [
              normalizedName,
              [entry.command, ...(entry.args ?? [])].map(value =>
                expandBundledAgentCommandValue(value.trim(), agentCommandPaths),
              ),
            ],
          ]
        : [];
  ...
  return {
    ...
    startupProbe: normalized.startupProbe ?? false,
    claudeExecutable: normalized.claudeExecutable,
