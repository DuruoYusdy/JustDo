...
import type { CompleteAcpxRuntime, CompleteAcpRuntimeTurn } from './runtime-proxy.js';
import {
...
const AGENT_COMMAND_IDENTITY_SAFE_ARG_RE = /^[A-Za-z0-9_@%+=:,./^~-]+$/u;

function normalizeAgentCommand(command: AcpxAgentCommand): string | undefined {
  const normalized = Array.isArray(command)
    ? command
        .map(part => (AGENT_COMMAND_IDENTITY_SAFE_ARG_RE.test(part) ? part : JSON.stringify(part)))
        .join(' ')
    : command;
  return normalized.trim() || undefined;
}

function appendAgentCommandArgs(command: AcpxAgentCommand, args: string[]): AcpxAgentCommand {
...
function appendCodexAcpConfigOverrides(
  ...
  return appendAgentCommandArgs(command, [OPENCLAW_CODEX_CONFIG_ARG, JSON.stringify(config)]);
}

function createModelScopedAgentRegistry(params: {
  agentRegistry: AcpAgentRegistry;
  scope: AsyncLocalStorage<CodexAcpModelOverride | undefined>;
  leaseCommand: (command: AcpxAgentCommand) => AcpxAgentCommand;
}): AcpAgentRegistry {
  return {
    resolve(agentName: string): AcpxAgentCommand {
      const command = params.agentRegistry.resolve(agentName);
      const commandIdentity = normalizeAgentCommand(command) ?? '';
      const override = params.scope.getStore();
      if (
        !override ||
        normalizeAgentName(agentName) !== CODEX_ACP_AGENT_ID ||
        !isCodexAcpCommand(commandIdentity)
      ) {
...
export class AcpxRuntime implements CompleteAcpxRuntime {
  ...
  private commandWithLaunchLease(command: AcpxAgentCommand): AcpxAgentCommand {
    const launch = this.launchLeaseScope.getStore();
    if (!launch) {
      return command;
    }
    if (normalizeAgentCommand(command) === launch.stableCommand) {
      const resolvedIdentity = readAcpxProcessLeaseIdentity(launch.resolvedCommand);
      return resolvedIdentity
        ? withAcpxLeaseEnvironment({
            command,
            leaseId: resolvedIdentity.leaseId,
            gatewayInstanceId: resolvedIdentity.gatewayInstanceId,
          })
        : command;
    }
  ...
  /** Probe one configured agent through its resolved ACP command and initialize handshake. */
  async doctorAgent(agentName: string): Promise<AcpRuntimeDoctorReport> {
    const normalizedAgentName = normalizeAgentName(agentName);
    if (!normalizedAgentName) {
      return {
        ok: false,
        code: 'ACP_AGENT_INVALID',
        message: 'ACP agent id is required.',
      };
    }
    ...
    const useBridgeSafeDelegate =
      this.managedToolsMcpBridgeEnabled || shouldUseBridgeSafeDelegateForCommand(command);
    const probeDelegate = new BaseAcpxRuntime(
      {
        ...this.delegateOptions,
        probeAgent: normalizedAgentName,
        ...(useBridgeSafeDelegate ? { mcpServers: [] } : {}),
      },
      this.delegateTestOptions,
    );
    const report = await this.runWithLaunchLease({
      sessionKey: ACPX_PROBE_LEASE_SESSION_KEY,
      command,
      finalizeCompletedProbe: true,
      run: () => probeDelegate.doctor(),
    });
    const sanitize = (value: string, maxChars: number) =>
      redactSensitiveText(value).slice(0, maxChars);
    return {
      ok: report.ok,
      ...(report.code ? { code: report.code } : {}),
      message: sanitize(report.message, 2_000),
      ...(report.details?.length
        ? { details: report.details.slice(0, 8).map(detail => sanitize(detail, 2_000)) }
        : {}),
    };
  }
      ...
      codexModelOverride && command
        ? normalizeAgentCommand(
            appendCodexAcpConfigOverrides(
              this.agentRegistry.resolve(normalizeAgentName(input.agent) ?? CODEX_ACP_AGENT_ID),
              codexModelOverride,
            ),
          )
        : command;
