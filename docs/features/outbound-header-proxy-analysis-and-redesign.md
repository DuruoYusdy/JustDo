...
手动“重建索引”通过 OpenClaw 原生 `force: true`、`reason: "cli"` 意图触发。runtime patch `009`
只在这个组合下不向 shadow database 复制旧 embedding cache，避免未变化文档被缓存完全短路；
provider/search 自动恢复即使使用 `force: true` 也继续复用 cache。
