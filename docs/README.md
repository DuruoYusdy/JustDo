...
- OpenClaw Gateway 版本由 `package.json.openclaw.version` 固定为 `v2026.9.2`。运行时补丁位于 `scripts/patches/v2026.9.2/`，补丁目录 README 是补丁清单的最终依据。
...
[OpenClaw Runtime Patch Guide](patches/openclaw-patch-guide.md) 说明补丁来源、顺序、验证、升级和故障定位。具体能力对应哪个补丁，必须以 `scripts/patches/v2026.9.2/README.md` 与 manifest/测试为准，旧版本目录仅用于追溯。
