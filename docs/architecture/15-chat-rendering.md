...
主输入框将文本、文件/图片、浏览器标注和录制视为独立消息内容。按钮与提交入口使用同一内容判定；附件准备失败时保留草稿。新建会话 IPC 与执行适配器允许有效附件独立发送，首条纯附件消息也进入即时显示流程。侧聊仍保持文本输入约束。“目标完成后继续改进”需要文字定义后续目标；仅有附件时不提交，也不清除已完成目标。

新会话标题优先取用户文本；没有文本时使用标注备注、页面标题、录制标题或附件文件名作为标题生成依据。只向标题生成器提供可读元数据，不传浏览器上下文封装、图片 Base64 或本地完整路径。立即展示该依据的首行（最多 50 字符），后台生成简短标题；缺少可读元数据时使用本地化的“附件会话”。
