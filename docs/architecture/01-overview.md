...
| ----------------- | ------------------------------------------------------------------ |
| 应用              | `v2026.8.12`                                                       |
| OpenClaw runtime  | `v2026.9.2`，按平台安装/同步、bundle、patch、prune 后打包          |
| Electron          | 42.6 系列                                                          |
...
定时任务 UI 映射到 OpenClaw 原生 `cron.*` API。Agent-turn 任务默认由 `main` 执行，也可选择已有助手，并使用独立会话；任务定义和 run history 的权威是 Gateway。JustDo 额外把已观察到的 run 写入本地结果收件箱，提供未读、分页、删除和 session 跳转。
