...
// Target: openclaw@2026.9.2 scoped session-access registry source/bundle.
...
const MARKER = 'JUSTDO_SCOPED_SESSION_ACCESS_V2026_9_2';
const replacement = `scopedSessionAccessProviders = globalThis[Symbol.for("${MARKER}")] ??= new Set();`;
function transform(content) {
  if (content.includes(MARKER)) {
    if (content.split(MARKER).length !== 2 || !/scopedSessionAccessProviders = globalThis\[(?:\/\* @__PURE__ \*\/ )?Symbol\.for\("JUSTDO_SCOPED_SESSION_ACCESS_V2026_9_2"\)\] \?\?= (?:\/\* @__PURE__ \*\/ )?new Set\(\);/.test(content)) throw new Error('Partial scoped access registry patch; rebuild pristine runtime.');
    return content;
  }
  const pattern = /scopedSessionAccessProviders = (?:\/\* @__PURE__ \*\/ )?new Set\(\);/g;
