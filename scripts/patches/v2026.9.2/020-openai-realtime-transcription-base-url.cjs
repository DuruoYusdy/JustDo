...
// Target: openclaw@2026.9.2 OpenAI realtime transcription provider factory.
...
const MARKER = 'JUSTDO_OPENAI_REALTIME_TRANSCRIPTION_BASE_URL_V2026_9_2';
...
function assertTargetShape(files) {
  if (files.length !== 1) {
    throw new Error(
      `OpenAI realtime transcription base URL target count is ${files.length}, expected 1`,
    );
  }
}
