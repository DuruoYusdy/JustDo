...
// Target: openclaw@2026.9.2 sessions.fork schema, Gateway handler, and SQLite cut resolver.
...
const MARKERS = {
  schema: 'JUSTDO_MANAGED_SESSION_FORK_TARGET_KEY_SCHEMA_V2026_9_2',
  handler: 'JUSTDO_MANAGED_SESSION_FORK_TARGET_KEY_HANDLER_V2026_9_2',
  accessor: 'JUSTDO_MANAGED_SESSION_FORK_ASSISTANT_ENTRY_V2026_9_2',
};
...
function transformHandler(content, filePath) {
  ...
  const forkCallPattern =
    /(forkSessionAtMessage\(\s*\{\s*\.\.\.mutationParams,\s*entryId,\s*targetKey(?:\s*:\s*[\w$]+)?,)(\s*creation\s*:)/u;
...
function transformAccessor(content, filePath) {
  ...
  const alreadyPatched =
    ...
    /includeTargetEntry\s*\?\s*(?:undefined|void\s+0)\s*:\s*extractEditorText/u.test(content) &&
  ...
  body = body.replace(
    editorPattern,
    [
      `const editorText = includeTargetEntry ? undefined : extractEditorText(${editorMessageName}.content);`,
  ...
  const editorTextPattern = /editorText:\s*extractEditorText\([\w$]+\.content\),/u;
