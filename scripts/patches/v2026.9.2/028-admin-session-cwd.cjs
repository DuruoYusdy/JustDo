...
// Target: openclaw@2026.9.2 sessions.create handler source/bundle.
...
const MARKER = 'JUSTDO_ADMIN_SESSION_CWD_V2026_9_2';
const NATIVE = 'sessionCwd && !requestedExecNode && (requestedProjectId || p.worktree !== true)';
const PATCHED_PATTERN = new RegExp(
  `sessionCwd\\s*&&\\s*!requestedExecNode\\s*&&\\s*!clientScopes\\.includes\\(ADMIN_SCOPE\\)\\s*&&\\s*\\(requestedProjectId\\s*\\|\\|\\s*[A-Za-z_$][\\w$]*\\.worktree\\s*!==\\s*true\\)\\s*\\/\\*${MARKER}\\*\\/`,
);
const NATIVE_PATTERN =
  /sessionCwd\s*&&\s*!requestedExecNode\s*&&\s*\(requestedProjectId\s*\|\|\s*([A-Za-z_$][\w$]*)\.worktree\s*!==\s*true\)/;
...
function transform(content, filePath = '<runtime>') {
  ...
  return content.replace(
    ...
    (_match, paramsName) =>
      ...
      `(requestedProjectId || ${paramsName}.worktree !== true) /*${MARKER}*/`,
  );
}
