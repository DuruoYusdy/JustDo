...
// Target: openclaw@2026.9.2 SQLite transcript history/reset-window projection.
...
const MARKERS = {
  history: 'JUSTDO_RESET_DISPLAY_HISTORY_V2026_9_2',
  window: 'JUSTDO_RESET_DISPLAY_WINDOW_V2026_9_2',
};
...
function transformHistoryProjection(content, filePath) {
  ...
  const originalPattern = new RegExp(
    `((?:(?:const|let|var)\\s+|,)\\s*[A-Za-z_$][\\w$]*\\s*=\\s*)([A-Za-z_$][\\w$]*)\\.findLastIndex\\(\\s*\\(?([A-Za-z_$][\\w$]*)\\)?\\s*=>\\s*\\3\\.event_type\\s*===\\s*(?:"reset"|'reset'|\\x60reset\\x60)\\)`,
    'gu',
  );
  ...
  const updatedBody = range.body.replace(
    originalPattern,
    (_match, prefix, rowsName) =>
      `${prefix}/^agent:[^:]+:justdo:/u.test(${range.firstParameter}.resolved.sessionKey ?? '') ? -1 : ` +
      `${rowsName}.findLastIndex((row) => row.event_type === "reset")` +
      markerSuffix(filePath, MARKERS.history),
  );
