...
// Target: openclaw@2026.9.2 cron schema, payload merge, RPC handlers and isolated runner.
...
const PREFIX = 'JUSTDO_CRON_SESSION_PERMISSION_V2026_9_2';
...
function handlers(text) {
  for (const method of ['add', 'update', 'run']) {
    ...
    const check = (job, explicit) =>
      `if (!justDoCronPermissionGuard(${job}, ${clientName}, readCronCallerScope(${clientName}), respond, ${explicit})) return;`;
    if (method === 'add') {
...
function transform(text, seam) {
  ...
  const normalized = value => {
    ...
    const helper = /functionjustDoCronPermissionGuard\(job,(client\d*),scoped,respond,explicit=false\)\{[^]*?returntrue;\}/.exec(code);
    if (helper) {
      const canonicalHelper = helper[0].replace(new RegExp(`\\b${helper[1]}\\b`, 'g'), 'client');
