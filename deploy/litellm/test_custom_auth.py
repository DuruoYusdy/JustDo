...
def test_environment_rename_preserves_old_settings_and_explicit_new_values(monkeypatch):
    monkeypatch.delenv('LITELLM_JWT_AUDIENCE', raising=False)
    monkeypatch.setenv('JUSTDO_JWT_AUDIENCE', 'existing-audience')
    assert custom_auth._auth_env('LITELLM_JWT_AUDIENCE') == 'existing-audience'
    ...
    monkeypatch.setenv('JUSTDO_DEFAULT_TEAM_ID', 'old-default')
    monkeypatch.setenv('LITELLM_DEFAULT_TEAM_ID', '')
    assert custom_auth._auth_env('LITELLM_DEFAULT_TEAM_ID') == ''
