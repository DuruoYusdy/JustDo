...
from shared.proxy_hooks.activity import ActivityHook, authenticate_jwt, validate_event, wrap
...
async def test_activity_uses_the_same_jwt_and_team_validator(monkeypatch):
    ...
    scope = {'type': 'http', 'path': '/customer/activity', 'method': 'POST', 'headers': [
        (b'x-justdo-jwt', b'token'), (b'x-user-account', b'alice'),
        (b'authorization', b'Bearer justdo-jwt-auth')]}
    assert await authenticate_jwt(scope) == 'alice'
    assert validator.call_args.args[0].headers['x-justdo-jwt'] == 'token'
