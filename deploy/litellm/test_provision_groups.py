...
def test_admin_client_does_not_forward_master_key_across_redirects():
    ...
    try:
        ...
        with pytest.raises(provision_groups.LiteLLMApiError) as error:
...
def test_team_update_preserves_metadata_and_marks_managed_ownership():
...
def test_assignment_removes_only_other_managed_teams():
    client = LiteLLMAdminClient("http://litellm.test", "server-master-key")
    client.request = Mock(return_value={})
