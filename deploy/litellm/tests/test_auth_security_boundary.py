...
async def test_invalid_identity_never_reaches_enrollment(monkeypatch, keys, failure):
    ...
    request = Request({"type": "http", "method": "GET", "path": "/v1/models", "headers": [
        (b"x-access-jwt", token.encode()),
        (b"x-user-account", b"other-user" if failure == "account" else b"new-user"),
    ]})
    expected = {
        'signature': 'JWT-1004', 'issuer': 'JWT-1007', 'audience': 'JWT-1008',
        'expired': 'JWT-1005', 'future': 'JWT-1006', 'lifetime': 'JWT-1011',
        'missing-jti': 'JWT-1009', 'missing-kid': 'JWT-1003',
        'symmetric': 'JWT-1003', 'account': 'JWT-1012',
    }
    with pytest.raises(ProxyException) as rejected:
        await custom_auth.user_api_key_auth(request, "Bearer access-jwt-auth")
    assert rejected.value.to_dict()['code'] == expected[failure]
    assert token not in str(rejected.value.to_dict())
