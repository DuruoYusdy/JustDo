...
from custom_auth import provision_first_login
...
def fixture(existing=None, **overrides):
    team = {"metadata": {"justdo_managed": True}, "blocked": False,
...
@pytest.mark.parametrize('overrides', [
    ...
    {"metadata": {"justdo_managed": True, "justdo_legacy_clients": True}},
])
...
async def test_concurrent_user_creation_does_not_overwrite_membership():
    db, tx = fixture()
    tx.query_raw.side_effect = [[{"metadata": {"justdo_managed": True},
...
async def test_default_member_budget_and_models_are_inherited_atomically():
    db, tx = fixture(
        metadata={"justdo_managed": True, "team_member_budget_id": "template"},
...
async def test_missing_default_member_budget_rolls_back_enrollment():
    db, tx = fixture(metadata={"justdo_managed": True, "team_member_budget_id": "missing"})
    tx.litellm_budgettable = SimpleNamespace(find_unique=AsyncMock(return_value=None))
    with pytest.raises(ProxyException, match='member budget is unavailable'):
        await provision_first_login(db, 'new-user', 'default-team')
    tx.execute_raw.assert_not_awaited()
