...
import { LOCAL_ASR_MODEL_IDS, type LocalAsrModelId } from '@shared/localAsr';
...
const ASR_MODEL_KEYS: Record<LocalAsrModelId, string> = {
  'sherpa-onnx-whisper-tiny': 'voiceAsrModelWhisperTiny',
...
const ModelStatus: React.FC<{
  ...
  onRetry: () => void;
  onRemove: () => void;
}> = ({ status, loading, onRetry, onRemove }) => {
  ...
  return (
    <div className="flex items-center gap-2 text-xs text-secondary">
      ...
      {status?.id ? (
        <code className="rounded bg-surface-raised px-1.5 py-0.5">{status.id}</code>
      ) : null}
      {status?.downloadBytes ? (
        <span>{`${status.downloadBytesExact === false ? '≈' : ''}${(status.downloadBytes / 1024 / 1024).toFixed(1)} MiB`}</span>
      ) : null}
      {status?.phase === 'error' || status?.phase === 'not-installed' ? (
        <button
          type="button"
          onClick={onRetry}
        ...
        >
          {status.phase === 'error'
            ? i18nService.t('retry')
            : i18nService.t('voiceModelDownload')}
        </button>
      ) : null}
      {available ? (
        <button type="button" onClick={onRemove} className="font-medium text-secondary hover:underline">
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-5 rounded-xl border border-border bg-surface p-5">
        <div className="flex items-start justify-between gap-6">
          ...
          <SettingSwitch
            ...
            onChange={inputEnabled => {
              update({ inputEnabled });
              if (inputEnabled && !asrStatus?.installed) {
                install(LocalSpeechModelKind.Asr, value.asrModelId);
              }
            }}
          />
        </div>
        ...
        />
        <div className={`border-t border-border pt-5 ${value.inputEnabled ? '' : 'opacity-50'}`}>
          <div className="mb-5 flex items-center justify-between gap-8">
            ...
            <div className="w-52 shrink-0">
              <ThemedSelect
                ...
                onChange={asrModelId => {
                  update({ asrModelId: asrModelId as LocalAsrModelId });
                  install(LocalSpeechModelKind.Asr, asrModelId);
                }}
          ...
          <div className="flex items-center justify-between gap-8">
            <div>
              <label htmlFor="voice-input-language" className="text-sm font-medium text-foreground">
      ...
      <section className="space-y-5 rounded-xl border border-border bg-surface p-5">
        <div className="flex items-start justify-between gap-6">
          ...
          <SettingSwitch
            ...
            onChange={outputEnabled => {
              ...
              if (outputEnabled && !ttsStatus?.installed) {
                install(LocalSpeechModelKind.Tts, value.ttsModelId);
              }
            }}
          />
        </div>
        ...
        />
        <div className={`border-t border-border pt-5 ${value.outputEnabled ? '' : 'opacity-50'}`}>
          <div className="mb-5 flex items-center justify-between gap-8">
            <div>
            ...
            <div className="w-52 shrink-0">
              <ThemedSelect
                ...
                onChange={ttsModelId => {
                  ...
                  install(LocalSpeechModelKind.Tts, ttsModelId);
                }}
      ...
      <p className="text-xs leading-5 text-muted">{i18nService.t('voicePrivacyNote')}</p>
    </div>
  );
};
