...
test('keeps unclassified user-visible Hooks before system-owned groups', () => {
  const groups = groupHooksBySource([
    createHook('bundled', 'openclaw-bundled'),
    createHook('other', 'future-source'),
  ]);

  expect(groups.map(group => group.id)).toEqual([HookGroupId.OTHER, HookGroupId.BUNDLED]);
});
