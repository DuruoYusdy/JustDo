...
vi.mock('@/features/plugins/components/extensions/ExtensionsManager', () => ({
  default: (props: { searchQuery: string; visibility: string }) => (
    <div data-testid="extensions-manager">{JSON.stringify(props)}</div>
  ),
}));
vi.mock('@/features/plugins/components/hooks/HookManager', () => ({
  default: () => <div>hooks-manager</div>,
}));
vi.mock('@/features/plugins/components/mcp/McpManager', () => ({
...
vi.mock('@/features/plugins/components/skills/SkillsManager', () => ({
